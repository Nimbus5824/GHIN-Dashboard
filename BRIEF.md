# My Links - Project Brief
A golf related tracker in the form of a dashboard. One page, clean design, mobile first. Modern font and elegant visual design.
Dark mode preferred with contrasting colorway and appropriate contrast ratios.

## Tech
Pure HTML, CSS, and JacaScript - no frameworks, no build tool
Single Index.html file
Deploys to Vercel as a static site

## Design
Multiple card layout, max width 480px
Fariways is the name of the app, complimentary neon green and turqoise colorways with white lettering
Weather tracker app for your region
This dashboard will track current handicap, scoring average, rounds played using GHIN as the main source of data and statistics
Simple line and bar graphs to cleanly depict player statistics with a warm color that plays well with the primary green colorway
A secondary function of the app will be a bag organizer where the user can log in what club he possesess and uses along with the hitting yardage, brand, loft, and other details
Smooth hover animations on the link buttons

## Links
GHIN Official site (https://www.ghin.com/)

## Nice to have
Beautiful backsplashes
iconography
elegant design with ample negative space between elements